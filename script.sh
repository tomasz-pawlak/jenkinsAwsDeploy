...
aws ecs run-task --launch-type="FARGATE" --cluster "${CLUSTER_NAME}" --task-definition "${TASK_DEFINITION_NAME}":"${REVISION}" --network-configuration '{ "awsvpcConfiguration": { "assignPublicIp":"ENABLED", "securityGroups": ["sg-05c1278be78a24802"], "subnets": ["subnet-08e975740a21718f9","subnet-020c2d5c819ada885","subnet-04c0d824983dce555"]}}'
